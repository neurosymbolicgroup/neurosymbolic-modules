...
def rl_prog_solves(program: Sequence[SynthEnvAction], task: Task) -> bool:
    env = SynthEnv(
        task=task,
        max_actions=len(program),
    )
    agent = rl.agent.ProgrammableAgent(program)
...
def rl_prog_solves2(program: Sequence[Tuple[Op, Tuple[int, ...]]],
                    task: Task) -> bool:
    env = SynthEnv(
        task=task,
        max_actions=len(program),
    )
    agent = rl.agent.ProgrammableAgent2(program)
